...
class PlayState extends MusicBeatState
{
	...
	var postSongDialogue:DialogueBox;
	...
	public static var guitaredition:Bool = false;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'tweet-tweet':
				dialogue = CoolUtil.coolTextFile(Paths.txt('tweet-tweet/dialogue' + difficulty));
			case 'nest':
				dialogue = CoolUtil.coolTextFile(Paths.txt('nest/dialogue' + difficulty));
				FlxG.save.data.hasSeenNest = true;
			case 'featherlord':
				dialogue = CoolUtil.coolTextFile(Paths.txt('featherlord/dialogue' + difficulty));
				afterDialogue = CoolUtil.coolTextFile(Paths.txt('featherlord/post/dialogue' + difficulty));
			case 'tutori-joel':
				dialogue = CoolUtil.coolTextFile(Paths.txt('tutori-joel/dialogue' + difficulty));
		}
	  ...
	  case 'darkJoel': 
		  {
			  defaultCamZoom = 0.9;
			  curStage = 'darkJoel';
			  /*var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('joelDarkStage', 'bonusWeek'));
			  var bgTex = Paths.getSparrowAtlas('joelDarkStage', 'bonusWeek');
			  bg.frames = bgTex;
			  bg.antialiasing = true;
			  ...
			  */
			  ...
			  bg.frames = Paths.getSparrowAtlas('joelDarkStage','bonusWeek');
			  bg.animation.addByPrefix('idle', 'Idle', 16);
			  bg.animation.addByPrefix('lightning', 'Background Whitty Moving', 16);
			  bg.animation.play('idle');
			  bg.scrollFactor.set(0.9, 0.9);
      ...
      case 'alley' | 'ballisticAlley':
		...
		postSongDialogue = new DialogueBox(false, afterDialogue);
		postSongDialogue.scrollFactor.set();
		postSongDialogue.finishThing = endSong;
		...
		doof.cameras = [camHUD];
		postSongDialogue.cameras = [camHUD];
		...
		if (isStoryMode)
			{
				...
				if (curSong.toLowerCase() == 'featherlord')
	...
	function afterSongDial(?dialogueBox:DialogueBox){
		trace("endCutscene");
		inCutscene = true;
		add(dialogueBox);
	...
	function featherIntro(?dialogueBox:DialogueBox):Void
	{
		var senpaiEvil:FlxSprite = new FlxSprite(100, -100);
		senpaiEvil.frames = Paths.getSparrowAtlas("cuttinDeezeBalls", "bonusWeek");
		senpaiEvil.animation.addByPrefix("idle", "Whitty Ballistic Cutscene");
		senpaiEvil.setGraphicSize(Std.int(senpaiEvil.width * 6));
		senpaiEvil.updateHitbox();
		add(senpaiEvil);
	...
	function startSong():Void
	{
		...
		if (!paused)
		{
			...
			if(guitaredition == true && PlayState.SONG.song.toLowerCase() == 'featherlord'){
				FlxG.sound.playMusic(Paths.inst('featherlord-guitar', ''), 1, false);
			} else {
				FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song, difficulty), 1, false);
			}
		}

		FlxG.sound.music.onComplete = endSongDialogue;
		vocals.play();
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices){
			...
			vocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song, difficulty));
		} else {
			vocals = new FlxSound();
		}
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var daNoteFirey:Bool = (songNotes[1] > 7);

				var gottaHitNote:Bool = section.mustHitSection;

				if (songNotes[1] > 3 && songNotes[1] < 7)
				{
					gottaHitNote = !section.mustHitSection;
				} else if(songNotes[1] < 4 && songNotes[1] < 7){
					gottaHitNote = section.mustHitSection;
				}
				else if (songNotes[1] > 7 && songNotes[1] < 12){
					gottaHitNote = section.mustHitSection;
				} else {
					gottaHitNote = !section.mustHitSection;
				}
				...
				var swagNote:Note = new Note(daNoteFirey, daStrumTime, daNoteData, oldNote);
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(false, daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, oldNote, true);
	...
	override public function update(elapsed:Float)
	{
		...
		if (dad.curCharacter == "whittyCrazy"){
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection || curSong.toLowerCase() == 'featherlord')
			{
		...
		if (health <= 0)
		{
			if(SONG.song.toLowerCase() == 'tutori-joel'){
				health += 0.1;
			} else {
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
					...
					if (storyPlaylist.length <= 0)
					{
						...
						if (SONG.validScore)
						{
							...
							if(PlayState.SONG.song.toLowerCase() == 'featherlord'){
								FlxG.save.data.beatFeatherlordHard = true;
							}
						}
				...
				trace(PlayState.storyPlaylist[0].toLowerCase() + difficulty);
					...
					//switch(SONG.song.toLowerCase()){
						//case 'featherlord':
						//	LoadingState.loadAndSwitchState(new VideoState("assets/videos/test/yourvideo.webm", new PlayState(), 0, false));
						//default:
								...
								if (coolNote.burning)
									{
										...
										if (curSong.toLowerCase() == 'tutori-joel' || curSong.toLowerCase() == 'tweet-tweet')
										{
											health += 0.45;
											//totalDamageTaken -= 0.45;
											//interupt = true;
											coolNote.wasGoodHit = true;
											coolNote.canBeHit = true;
											coolNote.kill();
											notes.remove(coolNote, true);
											coolNote.destroy();
										} 
										else if(curSong.toLowerCase() == 'nest')
										{
											health += 0.2;
											//totalDamageTaken -= 0.45;
											//interupt = true;
										...
										else if(curSong.toLowerCase() == 'featherlord' && (storyDifficulty != 2 || isDebug))
											{
												health -= 0.45;
												//totalDamageTaken += 0.45;
												//interupt = true;
	...
	*/
	...
	function lightningStrikeJoel():Void
	{
			FlxG.sound.play(Paths.soundRandom('thunder_', 1, 2));
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'featherlord' && curBeat >= 188 && curBeat < 192 && FlxG.camera.zoom > 0.005){
			switch(curBeat){
				case 188:
					FlxG.camera.zoom -= 0.203;
					camHUD.zoom -= 0.13;
				case 189:
					FlxG.camera.zoom -= 0.115;
					camHUD.zoom -= 0.065;
				case 190:
					FlxG.camera.zoom -= 0.059;
					camHUD.zoom -= 0.043;
				case 191:
					FlxG.camera.zoom -= 0.032;
					camHUD.zoom -= 0.028;
			}
		}

		if (curSong.toLowerCase() == 'featherlord' && curBeat == 192){
			FlxG.camera.zoom = 1;
			camHUD.zoom = 1;
		}

		/*var beatDrop:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		beatDrop.scrollFactor.set();
		beatDrop.alpha = 0;

		if(curSong.toLowerCase() == 'featherlord'){
			if(curBeat >= 188 && curBeat < 193){
				remove(beatDrop);
				beatDrop.alpha = 0;
				switch(curBeat){
					case 188:
		...
		if (/*curBeat > 64 && curBeat < 70*/curBeat == 74 && curSong.toLowerCase() == 'tutori-joel')
			{
		...
		if (curSong.toLowerCase() == 'featherlord' && curBeat % 16 == 0)
		{
			lightningStrikeJoel();
			if(storyDifficulty != 0){
				//Don't shake lightning on Easy mode
				if(storyDifficulty == 1){
					//Shake screen, but less intense on Normal mode
					FlxG.camera.shake(0.02, 0.4);
					camHUD.shake(0.02, 0.4);
				} else {
					//Make the shake almost unplayable on Hard mode
					FlxG.camera.shake(0.06, 0.4);
					camHUD.shake(0.06, 0.4);
				}
			}
		}

		if (curSong.toLowerCase() == 'featherlord' && curBeat % 16 == 1)
		{
